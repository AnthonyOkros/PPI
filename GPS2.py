    ...
    if new_data:
        ...
        # Declare data to be float so that data types are the same and can be compared.  
          ...
          # color.  
            ...
            if float(speed)> 1.000 and float(speed) < 2.000: # Check value captured, if between 1 and 2
              pixels.fill(CYAN) # Display color CYAN on NeoPixels
              pixels.show()
              print ("around 1.5")
              print (speed)
            if float(speed)> 2.000 and float(speed) < 3.000:
              pixels.fill(BLUE)
              #pixels.show()
              print ("around 2.5")
              print (speed)
            if float (speed)> 3.000 and float(speed) < 4.000:
              pixels.fill(PURPLE)
              #pixels.show()
              print ("around 3.5")
              print (speed)
            if float (speed) > 4.000 and float(speed) < 5.000:
              pixels.fill(YELLOW)
              #pixels.show()
              print ("around 4.5")
              print (speed)
            if float (speed) > 5.000 and float(speed) < 6.000:
              pixels.fill(RED)
              #pixels.show()
              print ("around 5.5")
              print (speed)
            #time.sleep(0.5) # Run loop every 1 second
